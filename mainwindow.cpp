...
#include"SmtpClient-for-Qt/src/mimefile.h"
#include <QMessageBox>
...
QRegularExpression f("[A-Z][a-zA-Z]*"); // Regular expression to match string with first character uppercase and rest alphabetic
   ...
   // Expression régulière pour correspondre à un des mois de l'année en français

   QRegularExpression d("(Janvier|Février|Mars|Avril|Mai|Juin|Juillet|Août|Septembre|Octobre|Novembre|Décembre)");
   QValidator *moinValidator = new QRegularExpressionValidator(d, this);
   ui->lineEdit_moin->setValidator(moinValidator);

   QRegularExpression h("(Janvier|Février|Mars|Avril|Mai|Juin|Juillet|Août|Septembre|Octobre|Novembre|Décembre)"); // Expression régulière pour correspondre à une chaîne commençant par une majuscule suivie de minuscules
   QValidator *moinValidato = new QRegularExpressionValidator(h, this);
   ui->lineEdit_moin_2->setValidator(moinValidato);
...
int idDon = ui->lineEdit_idDon->text().toInt();
    ...
    float montant = ui->lineEdit_montant->text().toFloat();
    int idOeuvre = ui->lineEdit_idOeuvre->text().toInt();
    QString moin = ui->lineEdit_moin->text();

   associations assoc(idDon,type,destinataire,montant,idOeuvre,moin);
    if(assoc.ajouter()) {
           QMessageBox::information(this, "Succès", "Association ajoutée avec succès.");
           // Réafficher la liste après l'ajout
           //assoc.afficher();+
           bool ok;
                   QString adresseEmail = QInputDialog::getText(this, "Saisir adresse e-mail", "Veuillez saisir l'adresse e-mail :", QLineEdit::Normal, "", &ok);

                   if (ok && !adresseEmail.isEmpty()) {
                       // Envoyer un e-mail à l'adresse spécifiée
                       SmtpClient smtp("smtp-relay.brevo.com", 587, SmtpClient::TcpConnection);
                       smtp.setUser("chahedchacha84@gmail.com");
                       smtp.setPassword("9mAFhRD0HUWgwyjs");

                       MimeMessage message;
                       message.setSender(new EmailAddress("chahedchacha84@gmail.com", "Shahd Jouini"));
                       message.addRecipient(new EmailAddress(adresseEmail, ""));
                       message.setSubject("Bonjour,notification d'ajout de don");

                       MimeText text;
                       text.setText("Bonjour,"+destinataire+"\nUn nouveau don a été ajouté à notre système.\nMerci pour votre contribution.\nCordialement,");
...
void MainWindow::on_modifier_clicked()
{
    ...
    int idDon = ui->lineEdit_2->text().toInt();
        ...
        float montant = ui->price->text().toFloat();
        int idOeuvre = ui->idou->text().toInt();
        QString moin = ui->lineEdit_moin_2->text();
    associations assoc(idDon,type,destinataire,montant,idOeuvre,moin);
